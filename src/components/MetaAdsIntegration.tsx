      ...
      <div className="space-y-3">
        ...
        {message && (
          <div className={`flex items-center gap-2 p-3 rounded-lg border text-sm ${
            status === 'success' ? 'bg-green-600/10 border-green-600/30 text-green-400' : 'bg-destructive/10 border-destructive/30 text-destructive'
          }`}>
...
function SyncSection() {
  ...
  async function handleSync() {
    ...
    const { data: tokenData } = await supabase
      .from('app_settings').select('value').eq('key', 'meta_access_token').single();
        ...
        <div className="space-y-2 mt-4">
          {results.map((r, i) => (
            <div key={i} className={`flex items-start gap-2 p-2.5 rounded-lg border text-sm ${
              r.ok ? 'bg-green-600/10 border-green-600/30 text-green-400' : 'bg-destructive/10 border-destructive/30 text-destructive'
            }`}>
      ...
      <div className="flex-1 min-w-0">
        {editing ? (
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleRename} className="text-green-500 hover:text-green-400"><Check className="w-4 h-4" /></button>
