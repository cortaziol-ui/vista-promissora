  ...
  return (
    <div className={isVertical ? 'h-[calc(100vh-3.5rem-3rem)] flex flex-col gap-5 overflow-y-auto' : 'space-y-6'}>
      ...
      <div className={`grid ${isVertical ? 'grid-cols-3 gap-5' : `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 ${isSeller ? 'xl:grid-cols-6' : 'xl:grid-cols-7'} gap-4`}`}>
        ...
        <KpiCard title="% da Meta" value={`${localPctMeta.toFixed(1)}%`} subtitle={`Faltam ${Math.max(0, localMetaVendas - localTotalVendas)} vendas`} icon={<TrendingUp className={`${isVertical ? 'w-9 h-9' : 'w-5 h-5'} text-kpi-goal-pct`} />} glowClass="kpi-glow-pct" colorClass="bg-kpi-goal-pct/15" size={isVertical ? 'large' : 'default'} />
        <KpiCard title="Total Vendas" value={String(localTotalVendas)} icon={<ShoppingCart className={`${isVertical ? 'w-9 h-9' : 'w-5 h-5'} text-kpi-sales`} />} glowClass="kpi-glow-sales" colorClass="bg-kpi-sales/15" size={isVertical ? 'large' : 'default'} />
        <KpiCard title="Projeção" value={`${Math.round(projecao)} vendas`} icon={<BarChart3 className={`${isVertical ? 'w-9 h-9' : 'w-5 h-5'} text-kpi-projection`} />} glowClass="kpi-glow-projection" colorClass="bg-kpi-projection/15" size={isVertical ? 'large' : 'default'} />
        {!isSeller && (
          <KpiCard title="Faturamento" value={fmtFull(localFaturamento)} icon={<DollarSign className={`${isVertical ? 'w-9 h-9' : 'w-5 h-5'} text-kpi-revenue`} />} glowClass="kpi-glow-revenue" colorClass="bg-kpi-revenue/15" size={isVertical ? 'large' : 'default'} />
        )}
      </div>
                    ...
                    <tr key={stat.vendedor.id} className="border-b border-border/30 hover:bg-secondary/50 transition-colors">
                      ...
                      <td className={`${isVertical ? 'py-3 px-3' : 'py-3 px-2'}`}>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className={`font-medium text-foreground ${isVertical ? 'text-lg' : ''}`}>{stat.vendedor.nome}</p>
